...
# Add Kafka dependencies configuration dynamically
if [ -n "$KAFKA_BOOTSTRAP_SERVERS" ]; then
    setKey _common druid.extensions.loadList "[\"druid-kafka-indexing-service\"]"
    setKey _common druid.extensions.directory "$(pwd)/extensions"
    setKey kafka druid.kafka.announce.host "$HOSTNAME"
    setKey kafka druid.kafka.internal.host "$HOSTNAME"
    setKey kafka druid.kafka.internal.port "8082" # Change port number if needed
    setKey kafka druid.kafka.internal.protocol "http" # Change protocol if needed
    setKey kafka druid.kafka.bootstrap.servers "$KAFKA_BOOTSTRAP_SERVERS"
fi
